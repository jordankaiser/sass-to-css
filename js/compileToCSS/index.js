...
async function compileToCSS(scssFilePath, ignoreDirectoryPaths) {
  await processFiles(scssFilePath, ignoreDirectoryPaths);
  // await compile(file);
}
 ...
 */
async function processFiles(scssFilePath, ignoreDirectoryPaths) {
  ...
  for (const file of filesText) {
    if (file.text && file.path) {
      const fileWithComments = insertImport(file);
 ...
 * @return {string} The text with import inserted.
 */
function insertImport(text) {
  const importText = `@import '../variables/index.scss';\n`;
  return importText + text.text;
}
